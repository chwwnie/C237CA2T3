...
// fully save session to database first then redirect to the next page
...
// count unread notifications for logged-in users not logged in = 0 app.use = every page
app.use((req, res, next) => {
    if (!req.session.user) {
        res.locals.unreadCount = 0;
        return next();
    }
    //ask the database how many unread notifications this logged-in user has
...
const validateRegistration = (req, res, next) => {
    ...
    if (!username || !email || !password || !phone || !address) {
        ...
        //for saving details in the form
...
// GET - just displays the empty registration form
// req.flash to pull the old form data and 0 to unpack
app.get('/register', (req, res) => {
    res.render('register', { messages: req.flash('error'), formData: req.flash('formData')[0] });
});

// takes the details of the form to unpack
app.post('/register', validateRegistration, (req, res) => {
    const { username, email, password, phone, address } = req.body;
//store into database and password encrypt using SHA1
    ...
    db.query(sql, [username, email, password, phone, address, 'user'], (err, result) => {
        if (err) {
// Duplicate error and sent back to register with details
...
//check if password or email is empty or incorrect
app.post('/login', (req, res) => {
   ...
   // cross check with the details entered with the database if its correct
...
// Update the user's details in the database and also update the session data
    ...
    db.query('UPDATE applications SET stage = ?, decisionNotes = ? WHERE id = ?',
        [stage, decisionNotes, applicationId], (err, result) => {
            ...
            db.query(infoSql, [applicationId], (err, infoResults) => {
                ...
                //  Send the applicant a notification about the decision
                ...
                // Insert the notification into the database for the user
...
//View my notifications - opening the page marks them all as read
